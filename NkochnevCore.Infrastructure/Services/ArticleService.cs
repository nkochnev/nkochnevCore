...
namespace NkochnevCore.Infrastructure.Services
{
	public class ArticleService : IArticleService
	{
		...
		public ArticleDomain GetArticleDomain(string key)
		{
			...
			if (article == null)
			{
				throw new Exception(string.Format("Не найдена статья с ключом {0}", key));
			}
			return article;
		}
			...
			string seoDescription)
		{
			...
			if (article != null)
			{
				throw new Exception($"Статья с ключом {article.Key} уже создана");
			}
		...
		private ArticleDomain FillArticle(ArticleDomain article, string key, string title, string content, string preview, string seoKeyWords,
			string seoDescription)
		{
			article.Title = title;
